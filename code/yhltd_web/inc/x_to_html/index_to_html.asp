...
web_BottomMenu=""
set rs=server.createobject("adodb.recordset")
sql="select [name],[url],[id] from web_menu_type where BottomNav=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount

web_BottomMenu=web_BottomMenu&"<div class='box_240px_left'><div class='post'><h2><a href='"&rs("url")&"'>"&rs("name")&"</a> </h2> "
...
web_BottomMenu=web_BottomMenu&"</div></div> "
rs.movenext
next
end if
rs.close
...
'首页顶部幻灯广告读取替换
set rs=server.createobject("adodb.recordset")
sql="select top 10 name,url,image,ADcode from web_ads  where [position]=30 and view_yes=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
if rs("adcode")<>"" then
...
WebAboutContent=left(ClearHtml(rs("content")),105)
...
sql="select top 3 [title],file_path,[description],[time],[image] from [article] where ArticleType=1 and cid='"&ItemID&"' and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
...
WebNewNewsList=WebNewNewsList&"<div class='DivLi'>"
WebNewNewsList=WebNewNewsList&"<div class='DivLeft'><a href=''><img src='/images/up_images/"&rsp("image")&"' alt='"&rsp("title")&"'></a></div>"
WebNewNewsList=WebNewNewsList&"<div class='DivRight'>"
WebNewNewsList=WebNewNewsList&"<p class='Title'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),20)&"</a></p>"
WebNewNewsList=WebNewNewsList&"<p class='Des'>"&left(ClearHtml(rsp("description")),51)&"...</p>"
WebNewNewsList=WebNewNewsList&"</div>"
WebNewNewsList=WebNewNewsList&"<div class='clearfix'></div>"
WebNewNewsList=WebNewNewsList&"</div>"
...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 12 [name],[folder],[id],[pid],[ppid] from [category] where pid="&ItemID&" and ClassType=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
...
set rsp=server.createobject("adodb.recordset")
sql="select top 10 [title],file_path,[image] from [article] where ArticleType=2 and cid='"&ItemID&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
Block03_LeftItem=Block03_LeftItem&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"' alt='"&rsp("title")&"'/></a><div class='albumtitle'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),24)&"</a></div></div></div>"
rsp.movenext
loop
end if 
rsp.close
