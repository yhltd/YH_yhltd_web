...
<%
 '文章列表读取替换
sql="select [title] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
sql="select [id],[title],[content],[url],[description],[author],[cid],[file_path],[time],[comment],[hit] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
